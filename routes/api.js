...
router.post('/sessions', (req, res) => {
    ...
    if (!host || !host.civ) {
        return res.status(400).json({ error: 'host.civ ist erforderlich.' });
    }
...
router.post('/sessions/join', (req, res) => {
    ...
    if (!joinCode || !password || !player || !player.civ) {
        return res.status(400).json({ error: 'joinCode, password und player.civ sind erforderlich.' });
    }
